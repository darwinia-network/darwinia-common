...
impl DebugT for Debug {
	...
	fn trace_transaction(
		...
		log::debug!(
			"bear: --- enter Debug(trace_transaction), transaction_hash {:?}, params {:?}",
			transaction_hash,
			params
		);
	...
	fn trace_block(
		...
		log::debug!(
			"bear: --- enter Debug(trace_block), id {:?}, params {:?}",
			id,
			params
		);
		let mut requester = self.requester.clone();
	...
	C::Api: EthereumRuntimeRPCApi<B>,
{
	...
	pub fn task(
		...
		log::debug!(
			"bear: --- enter DebugHandler(task), permit pool: {:?}",
			permit_pool
		);
			...
			loop {
				match rx.next().await {
					Some((
						...
						log::debug!(
							"bear: ---- receive request, tx_hash {:?}, params {:?}",
							transaction_hash,
							params
						);
					...
					Some(((RequesterInput::Block(request_block_id), params), response_tx)) => {
						log::debug!(
							"bear: ---- receive request, request_block_id {:?}, params {:?}",
							request_block_id,
							params
						);
						let client = client.clone();
	...
	fn handle_params(params: Option<TraceParams>) -> RpcResult<(TracerInput, single::TraceType)> {
		log::debug!("bear: ---- enter handle_params), params {:?}", params);
	...
	fn handle_block_request(
		...
		log::debug!(
			"bear: --- enter DebugHandler(handle_block_request), id {:?} params {:?}",
			request_block_id,
			params
		);
		let (tracer_input, trace_type) = Self::handle_params(params)?;
		log::debug!(
			"bear: ---- handle_params return, tracer_input {:?}, trace_type {:?}",
			tracer_input,
			trace_type
		);
		...
		log::debug!("bear: ---- reference_id {:?}", reference_id);
		...
		let eth_tx_hashes = statuses
			...
			.collect();
		log::debug!("bear: ---- eth tx hashes {:?}", eth_tx_hashes);
		...
		return match trace_type {
			single::TraceType::CallList => {
				...
				log::debug!("bear: ---- proxy listener CallList using.... here ........");
				proxy.using(f)?;
	...
	fn handle_transaction_request(
		...
		log::debug!(
			"bear: --- enter DebugHandler(handle_transaction_request), hash {:?} params {:?}",
			transaction_hash,
			params
		);
		let (tracer_input, trace_type) = Self::handle_params(params)?;
		...
		log::debug!(
			"bear: ---- load transactions, hash {:?}, index {:?}",
			hash,
			index
		);
		...
		log::debug!("bear: ---- load hash, reference_id{:?}", reference_id);
		// Get ApiRef. This handle allow to keep changes between txs in an internal buffer.
		let api = client.runtime_api();
		// Get Blockchain backend
		let blockchain = backend.blockchain();
		...
		if let Some(block) = reference_block {
			...
			if let Some(transaction) = transactions.get(index) {
				log::debug!("bear: ---- get the actual transaction {:?}", transaction);
				let f = || -> RpcResult<_> {
					api.initialize_block(&parent_block_id, &header)
						.map_err(|e| internal_err(format!("Runtime api access error: {:?}", e)))?;

					// bear: --- the result is empty
					let _result = api
				...
				return match trace_type {
					single::TraceType::Raw {
						...
						log::debug!("bear: ---- proxy listener raw before raw {:?}", proxy);
						proxy.using(f)?;
						log::debug!("bear: ---- proxy listener raw after raw {:?}", proxy);
						Ok(Response::Single(
