...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		Ok(match id {
			"song" => Box::new(chain_spec::song_config()?),
			"song-dev" | "dev" => Box::new(chain_spec::song_development_config()),
			"song-genesis" => Box::new(chain_spec::song_build_spec_config()),
			path => Box::new(chain_spec::SongChainSpec::from_json_file(PathBuf::from(
				path,
			))?),
		})
	}
