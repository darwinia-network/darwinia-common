...
impl EthHeader {
	pub fn from_str_unchecked(s: &str) -> Self {
		...
		while let Some(s) = s.next() {
			...
			} else if s.starts_with("\"gas_limit") {
				eth_header.gas_limit = str_to_u64(parse_value_unchecked(s)).into();
			} else if s.starts_with("\"gas_used") {
			...
			} else if s.starts_with("\"log_bloom") {
			...
			} else if s.starts_with("\"receipts_root") {
				eth_header.receipts_root =
					fixed_hex_bytes_unchecked!(parse_value_unchecked(s), 32).into();
			} else if s.starts_with("\"uncles_hash") {
			...
			} else if s.starts_with("\"transactions_root") {
	...
	fn test_ropsten_header_bare_hash() {
		let header = EthHeader::from_str_unchecked(
			r#"
			{
				...
				"difficulty": 107253406,
				"extra_data": "0xd783010503846765746887676f312e372e33856c696e7578",
				"gas_limit": 4707881,
				"gas_used": 98984,
				"hash": "0xa83130084c3570d9e0432bbfd656b0fe6088d8837967ef552974de5e8dc1fad5",
				"logs_bloom": "",
				"number": 70000,
				"parent_hash": "0xe7a8c03a03f7c055599def00f21686d3b9179d272c8110162f012c191d303dad",
				"receipts_root": "0xfbbc5695aac7a42699da58878f0a8bb8c096ed95a9b087989c0903114650ca70",
				"seal": [
					"0xa0341e3bcf01c921963933253e0cf937020db69206f633e31e0d1c959cdd1188f5",
					"0x88475ddd90b151f305"
				],
				"state_root": "0x76565e67622936b6b9eac50f3a9ad940270f1c6d1d9f203fc6af4e0eb67b20fa",
				"timestamp": 1480533880,
				"transactions_root": "0x35ecd6e29d0b8d161bd7863cfa3198e979b451fa637834b96b0da3d8d5d081cf",
