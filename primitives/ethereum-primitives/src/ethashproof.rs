...
impl EthashProof {
	pub fn from_str_unchecked(s: &str) -> Self {
		...
		for e in s.splitn(60, '"') {
			let l = e.len();
			if l == 34 {
				proof.push(array_bytes::hex_str_array_unchecked!(e, 16).into());
			} else if l == 130 {
				dag_nodes.push(array_bytes::hex_str_array_unchecked!(e, 64).into());
			} else if l > 34 {
				// should not be here
	...
	pub fn apply_merkle_proof(&self, index: u64) -> H128 {
		fn hash_h128(l: H128, r: H128) -> H128 {
			...
			array_bytes::bytes_array_unchecked!(sha2_256(&data)[16..], 16).into()
		}
		...
		// `H256` is 32 length, truncate is safe; qed
		let mut leaf = array_bytes::bytes_array_unchecked!(sha2_256(&data)[16..], 16).into();
