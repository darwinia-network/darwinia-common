...
pub struct Header {
	...
	// #[cfg_attr(any(feature = "full-serde", test), serde(rename = "miner"))]
	pub author: Address,

	pub transactions_root: H256,
	// #[cfg_attr(any(feature = "full-serde", test), serde(rename = "sha3Uncles"))]
	...
	// #[cfg_attr(any(feature = "full-serde", test), serde(rename = "logsBloom"))]
	pub log_bloom: Bloom,
	...
	// #[cfg_attr(
	// 	any(feature = "full-serde", test),
	// 	serde(deserialize_with = "bytes_array_from_hex")
	// )]
	pub seal: Vec<Bytes>,
		...
		D: Deserializer<'de>,
	{
		...
		impl<'de> Visitor<'de> for HeaderVisitor {
				...
				V: MapAccess<'de>,
			{
				...
				// let mut uncles_hash = None;
				// let mut extra_data = None;
				// let mut state_root = None;
				// let mut receipts_root = None;
				// let mut log_bloom = None;
				...
				while let Some(key) = map.next_key()? {
					match key {
						Field::difficulty => {
							check_option!(difficulty);

							difficulty = Some(u64_from_hex(map.next_value::<&str>()?));
						}
						Field::extraData => {}
						Field::gasLimit => {}
						Field::gasUsed => {}
...
fn bytes_array_from_hex(hex: impl AsRef<str>) -> Result<Vec<Bytes>, impl fmt::Debug> {
	serde_json::from_str::<Vec<&str>>(hex.as_ref())?
		...
		.collect::<Result<_, _>>()
		.map_err(|e| serde_json::Error::custom(write!("{:?}", e)))
}
