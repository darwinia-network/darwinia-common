...
impl EthereumHeader {
	...
	pub fn from_scale_codec_str<S: AsRef<str>>(s: S) -> Option<Self> {
		if let Ok(eth_header) =
			<Self as Decode>::decode(&mut &array_bytes::bytes_unchecked(s.as_ref())[..])
		{
			Some(eth_header)
		} else {
			None
	...
	pub fn from_str_unchecked(s: &str) -> Self {
		...
		for s in s {
			...
			if s.starts_with("\"difficulty") {
				...
				eth_header.extra_data = array_bytes::bytes_unchecked(parse_value_unchecked(s));
				...
				eth_header.hash =
					Some(array_bytes::hex_str_array_unchecked!(parse_value_unchecked(s), 32).into());
				...
				eth_header.author =
					array_bytes::hex_str_array_unchecked!(parse_value_unchecked(s), 20).into();
			} else if s.starts_with("\"mixHash") {
				mix_hash = array_bytes::hex_str_array_unchecked!(parse_value_unchecked(s), 32).into();
			} else if s.starts_with("\"nonce") {
				nonce = array_bytes::hex_str_array_unchecked!(parse_value_unchecked(s), 8).into();
				...
				eth_header.parent_hash =
					array_bytes::hex_str_array_unchecked!(parse_value_unchecked(s), 32).into();
...
{
	Ok(array_bytes::bytes_unchecked(&String::deserialize(
		deserializer,
	)?))
}
	...
	D: serde::Deserializer<'de>,
{
	Ok(<Vec<String>>::deserialize(deserializer)?
		.into_iter()
		.map(|s| array_bytes::bytes_unchecked(&s))
		.collect())
}
...
mod tests {
	...
	fn deserialize_should_work() {
		...
		let encoded_header = array_bytes::bytes_unchecked("");
		assert_eq!(
