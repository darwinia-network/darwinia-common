...
pub(crate) fn compute_offchain_election<T: Trait>() -> Result<(), OffchainElectionError> {
	...
	let era = <Module<T>>::active_era()
		.map(|e| e.index)
		.unwrap_or_default();

	// send it.
	let call: <T as Trait>::Call =
		Call::submit_election_solution_unsigned(winners, compact, score, era).into();
	...
	ExtendedBalance: From<<OffchainAccuracy as PerThing>::Inner>,
{
	...
	let score = {
		let staked =
			sp_phragmen::assignment_ratio_to_staked(low_accuracy_assignment.clone(), |s| {
