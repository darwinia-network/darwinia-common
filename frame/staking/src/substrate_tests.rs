...
mod election_data_provider {
	...
	fn chill_other_works() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_noop!(
					Staking::chill_other(Origin::signed(1), 2),
					<Error<Test>>::CannotChillOther
				);
				assert_noop!(
					Staking::chill_other(Origin::signed(1), 4),
					<Error<Test>>::CannotChillOther
				);

				// Change the minimum bond
				assert_ok!(Staking::update_staking_limits(
					Origin::root(),
					1_500,
					2_000,
					None,
	...
	fn capped_stakers_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_ok!(create_validators::<Test>(max - validator_count, 100));

			// but no more
			let (_, last_validator) =
				create_stash_controller::<Test>(1337, 100, RewardDestination::Controller).unwrap();
			assert_noop!(
			...
			let (_, last_nominator) =
				create_stash_controller::<Test>(20_000_000, 100, RewardDestination::Controller)
					.unwrap();
			assert_noop!(
