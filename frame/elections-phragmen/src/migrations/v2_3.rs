//! Migrations to version [`2.3.0`], as denoted by the changelog.
...
/// The old prefix.
pub const OLD_PREFIX: &[u8] = b"DarwiniaPhragmenElection";
...
pub fn migrate<T: frame_system::Config, P: GetPalletVersion, N: AsRef<str>>(
	...
	match maybe_storage_version {
		Some(storage_version) if storage_version <= PalletVersion::new(2, 2, 0) => {
		...
		_ => {
			log::warn!(
				target: "runtime::elections-phragmen",
				"Attempted to apply migration to v2.3 but failed because storage version is {:?}",
				maybe_storage_version,
			);
			0
		}
...
pub fn pre_migration<P: GetPalletVersion, N: AsRef<str>>(new: N) {
	...
	assert!(<P as GetPalletVersion>::storage_version().unwrap().major == 2);
	assert!(<P as GetPalletVersion>::storage_version().unwrap().minor == 2);
}
...
pub fn post_migration<P: GetPalletVersion>() {
	...
	// ensure we've been updated to v2.3 by the automatic write of crate version -> storage version.
	assert!(<P as GetPalletVersion>::storage_version().unwrap().major == 2);
	assert!(<P as GetPalletVersion>::storage_version().unwrap().minor == 3);
}
