...
use merkle_mountain_range::{leaf_index_to_mmr_size, leaf_index_to_pos, Merge};
...
fn test_insert_header() {
	new_test_ext().execute_with(|| {
		...
		let pos = 19;
		assert_eq!(pos, block_number_to_pos(h1));
		...
		let mmr = MMR::<_, MMRMerge<Test>, _>::new(block_number_to_pos(h2), store);
...
#[test]
fn test_block_number_to_mmr_size_pos() {
	(90..120).for_each(|index| {
		assert_eq!(
			leaf_index_to_mmr_size(index),
			block_number_to_mmr_size(index)
		);

		assert_eq!(leaf_index_to_pos(index), block_number_to_pos(index));
	});
}
