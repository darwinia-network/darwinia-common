	...
	P: CreatePayload<T::AccountId, MultiSigner, MultiSignature>,
{
	fn outbound_latest_generated_nonce(
		...
		let lane_id = abi_decode_bytes4(data)
			.map_err(|_| helper.revert("S2s(OutboundLatestGeneratedNonce): decode failed"))?;
	...
	fn inbound_latest_received_nonce(
		...
		let lane_id = abi_decode_bytes4(data)
			.map_err(|_| helper.revert("S2s(InboundLatestReceivedNonce): decode failed"))?;
	...
	fn encode_unlock_from_remote_dispatch_call(
		...
		let unlock_info = S2sRemoteUnlockInfo::abi_decode(data).map_err(|_| {
			helper.revert("S2s(EncodeUnlockFromRemoteDispatchCall): decode unlock failed")
		})?;
		...
		.map_err(|_| {
			helper.revert("S2s(EncodeUnlockFromRemoteDispatchCall): decode remote unlock failed")
		})?;
	...
	fn encode_send_message_dispatch_call(
		...
		let params = S2sSendMessageParams::decode(data).map_err(|_| {
			helper.revert("S2s(EncodeSendMessageDispatchCall): decode send message info failed")
		})?;
		...
		.map_err(|_| {
			helper.revert("S2s(EncodeSendMessageDispatchCall): encode send message call failed")
		})?;
