...
fn read_complex_solidity_function() {
	...
	assert_eq!(
		...
		MultiLocation {
			parents: 1,
			interior: vec![
				Bytes::from(&hex2bytes_unchecked("00000003e8")[..]),
				Bytes::from(&hex2bytes_unchecked("0403")[..]),
			],
		}
	);
	...
	assert_eq!(
		...
		MultiLocation {
			...
			interior: vec![Bytes::from(
				&hex2bytes_unchecked(
					"01010101010101010101010101010101010101010101010101010101010101010100"
				)[..]
			)],
		}
	);
...
fn write_static_size_tuple() {
	let output =
		EvmDataWriter::new().write((Address(H160::repeat_byte(0x11)), U256::from(1u8))).build();
