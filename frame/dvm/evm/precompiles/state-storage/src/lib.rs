		...
		let output = match action {
			Action::StateGetStorage => {
				let tokens = ethabi::decode(&[ParamType::Bytes], data)
					.map_err(|_| helper.revert("StateStorage: ethabi decoded error"))?;
				let key = match &tokens[0] {
					Token::Bytes(bytes) => bytes,
					_ => return Err(helper.revert("StateStorage: The input param error")),
				};

				if key.len() < PALLET_PREFIX_LENGTH || !F::allow(&key[0..PALLET_PREFIX_LENGTH]) {
					return Err(helper
						.revert("StateStorage: This state of the module has read restriction"));
				}
