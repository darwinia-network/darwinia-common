...
impl<T: Config> RingBack<T> {
	...
	pub fn transfer(input: &[u8], _: Option<u64>, context: &Context) -> PrecompileResult {
		...
		let transfer_addr =
			array_bytes::hex_try_into(TRANSFER_ADDR).map_err(|_| ret_err("invalid address"))?;
		ensure!(source == transfer_addr, ret_err("Invalid context address"));
...
impl<T: frame_system::Config> InputData<T> {
	pub fn decode(data: &[u8]) -> Result<Self, PrecompileFailure> {
		...
		Err(ret_err("Invalid input data length"))
	}
}
