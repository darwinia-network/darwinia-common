...
fn test_extract_value_from_json_response() {
	let result_part =
		EthOffchain::extract_from_json_str(&SUPPOSED_SHADOW_FAKE_RESPONSE[..], b"result" as &[u8])
			.unwrap();
	assert_eq!(
		result_part,
	...
	let eth_header_part =
		EthOffchain::extract_from_json_str(result_part, b"eth_header" as &[u8]).unwrap();
	assert_eq!(eth_header_part, br#"eth-content"# as &[u8]);
	let proof_header_part =
		EthOffchain::extract_from_json_str(result_part, b"proof" as &[u8]).unwrap();
...
fn test_extract_value_from_non_order_json_response() {
	let result_part = EthOffchain::extract_from_json_str(
...
fn test_build_eth_header_from_json_response() {
	let eth_header_part = EthOffchain::extract_from_json_str(
		&SUPPOSED_SHADOW_JSON_RESPONSE[..],
		b"eth_header" as &[u8],
	)
	.unwrap_or_default();
	...
	let proof_part =
		EthOffchain::extract_from_json_str(&SUPPOSED_SHADOW_JSON_RESPONSE[..], b"proof" as &[u8])
			.unwrap_or_default();
...
fn test_build_eth_header_from_scale_response() {
	let eth_header_part = EthOffchain::extract_from_json_str(
		&SUPPOSED_SHADOW_SCALE_RESPONSE[..],
		b"eth_header" as &[u8],
	)
	.unwrap_or_default();
	...
	let proof_part =
		EthOffchain::extract_from_json_str(&SUPPOSED_SHADOW_SCALE_RESPONSE[..], b"proof" as &[u8])
			.unwrap_or_default();
