...
impl<T: Config> Relayable for Module<T> {
	...
	fn verify_relay_proofs(
		...
		let mmr_root = array_bytes::array!(mmr_root, 32).into();
		...
		if let Some(best_confirmed_block_number) = optional_best_confirmed_relay_header_id {
			...
			//  c: last comfirmed block 1st: 1st submit block
				...
				Self::verify_mmr(
					...
					mmr_proof
						.iter()
						.map(|h| array_bytes::array!(h, 32).into())
						.collect(),
					vec![(
			...
			// c: current submit  1st: 1st submit block
				...
				Self::verify_mmr(
					...
					vec![(
						header.number,
						array_bytes::array!(header.hash.ok_or(<Error<T>>::HeaderInv)?, 32).into(),
					)],
				),
				<Error<T>>::MMRInv
			);
		...
		let mmr_root = Self::confirmed_header_parcel_of(mmr_proof.last_leaf_index + 1)
			...
			Self::verify_mmr(
				...
				vec![(
					...
					array_bytes::array!(ethereum_header.hash.ok_or(<Error<T>>::HeaderHashInv)?, 32)
						.into(),
				)]
			),
			<Error<T>>::MMRInv
